...
def task1_favorite_genre():
    ...
    """
    per_genre = plays.groupBy("user_id", "genre").agg(count("*").alias("play_count"))
...
def task2_average_listen_time():
    ...
    """
    per_song = logs.groupBy("song_id").agg(
        sround(avg("duration_sec"), 2).alias("avg_duration_sec"),
        count("*").alias("play_count"),
    )
    return (per_song.join(songs.select("song_id", "title"), "song_id")
...
def task3_genre_loyalty(favorite):
    ...
    return (favorite.join(totals, "user_id")
...
def task4_night_owls():
    ...
    """
    return (logs.filter(hour("timestamp") < 5)
            .groupBy("user_id")
